...
type ArgumentKey = keyof typeof ARGUMENT;

const getFullCLIVariableName = (variableName: ArgumentKey): string => `--${variableName}`;
const getAbbreaviatedVariableName = (variableName: ArgumentKey): string => `-${variableName[0]}`;

const createGetBooleanValue = (variableName: ArgumentKey) => {
...
const createGetStringValue = (variableName: ArgumentKey) => {
  const fullName = getFullCLIVariableName(variableName);
  const abbreaviatedName = getAbbreaviatedVariableName(variableName);

  return (arg: string): string | null => {
...
export const getCLIVariables =
  (cliArgumentsRecord: typeof ADDITIONAL_CLI_ARGS) => (args: string[]) => {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      let value;
      const argumentName = cliArgumentsRecord.find(({ callback, argumentName }) => {
