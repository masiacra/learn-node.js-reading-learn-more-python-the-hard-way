...
Node.js - это опенсоурсная серверная среда выполнения реального времени, построенная на базе 
гугловского JavaScript движка V8. Node.js предоставляет основанную на событиях, неблокирующую
(асинхронную) I/O и кросс-платформенную среду выполнения реального времения доя построения высоко 
масштабируемых серверных приложений, использующих JavaScript.
...
Node.js запускается в одном процессе и код приложения запускается в одном потоке. Таким образом
нужно меньше ресурсов, чем для других аналогичных платформ.

Все запросы пользователя к вашему веб-приложению будут обработаны в одном потоке, и все работа с 
вводом-выводом или долгоидущие задания выполняются асинхронно для определеннго запроса. Т.о. этот
единственный поток (thread) не должен ожидать завершения запроса, а может обрабатывать следующий
запрос. Когда асинхронная работа по вводу-выводу завершается, то запрос обрабатывается дальше и 
отсылается ответ.
...
## Q. Какие ключевые особенности Node.js?

* **Асинхронный и управляемый событиями (Asynchronous and Event driven)** – все API в Node.js асинхронные. Эта особенность означает, что, если Node получает запрос на какую-либо операцию ввода/вывода, то будет выполнять это операцию где то на заднем плане и продолжит обработку других запросов. 
Таким образом он (Node.js) не ждет ответа от предыдущих запросов. 

* **Быстрый в исполнении кода** – Node.js использует движок выполнения JavaScript в реальном
времени, который используется в Google Chrome. Кроме того, Node имеет обертку вокруг JavaScript
движка, которая делает движок гораздо быстрее и, следовательно, обработка запросов внутри Node.js
также становится быстрее.

* **Однопоточный, но высокомасштабируемый** – Node.js использует однопоточную модель цикла событий.
Запросы от этих событий могут достигать, а могут не достигать сервера немедленно. Однако это не 
блокирует другие операции. Это делает Node.js высокомасштабируемым. Традиционные серверы создают
ограниченное количество потоков для обработки запросов, в то время, как Node.js создает
единственный поток, чтобы обслуживать гораздо большее количество таких запросов.

* **Библиотеки Node.js используют JavaScript** – это еще один важный аспект Node.js с точки зрения
разработчика. Большинство разработчки уже хорошо разбираются в JavaScript. Таким образом, 
разработка в Node.js становится проще для разработчиков, которые знают JavaScript.

* **Активное и полное жизни сообщество фреймворка Node.js** – активное сообщество всегда держит
фреймворк в актуальном состоянии в соответствии с последними трендами в веб-разработке.

* **Нет буфферизации** – приложения Node.js никогда не буфферизуют какие-либо данные. Они просто
...
Приложение Node.js создается создает единственный поток при вызове. Всякий раз, когда Node.js
получает запрос, то сперва завершает его обработку перед тем, как двигаться дальше.

Node.js работает асинхронно, используя цикл событий (event loop) и функции обратного вызова
(callback functions) для обработки множества запросов приходящих параллельно. Цикл событий (event
loop) - это функциональность, которая обрабатывает все ваши внешние события и просто конвертирует
их в функции обратного вызова. Цикл событий вызывает все обработчики событий в соответствующий
момент времени. Таким образом, множество работы проделывается "за кадром", и пока обрабатывается
единственный запрос, вновь приходящие запросы не вынуждены ждать незавершенную обработку.

Пока обрабатывается запрос, Node.js прикрепляет функцию обратного вызова к нему и двигает его на
задний план. Теперь, когда бы не был готов ответ, случается событие, которое триггерит
соответствующую функцию обратного вызова послать ответ.

<div align="right">
    <b><a href="#table-of-contents">↥ back to top</a></b>
</div>

## Q. Какая разница между процессом и потоком в Node.js? 

**1. Процесс:**

Процессы в сущности это программы, которые отправляются по состоянию готовности и ставятся в 
очередь на исполнение в CPU. Дескриптор процесса (PCB) содержит всю информацию о процессе. Процесс
может создать другой процесс, который будет именоваться дочерним. Процесс занимает больше времени
до завершения, и его изолированность означает, что он не делит память с каким-либо другим 
процессом.

Процесс может принимать следующие состояния: новый (new), готов к исполнению (ready), в процессе (running), в ожидании (waiting), завершенный terminated и висящий (suspended).
The process can have the following states new, ready, running, waiting, terminated, and suspended.

**2. Поток:**

Поток - это сегмент процесса, что означает, что процесс может иметь множество потоков и это множество потоков содержатся в процессе. Поток имеет три состояния: в процессе, готов и заблокирован.

Поток занимает меньше времени до заверщения в сравнении с процессом, но в отличие от процесса,
потоки не изолированы.
...
## Q. Как создать простой сервер в Node.js, который возвращает Hello world?

**Шаг 01**: Создайте директорию проекта
...
**Шаг 02**: Инициализируйте проект и привяжите его к npm

```js
npm init
```

Это создает файл `package.json` в вашей директории myapp. Файл содержит ссылки на все npm пакеты,
которые вы скачали для вашего проекта. Команда предложит ввести множество вещей. Вы можете
пропустить их все ЗА ИСКЛЮЧЕНИЕМ следующей:
...
**Шаг 03**: Установите Express в директории myapp

```js
npm install express --save
```

**Шаг 04**: В файле app.js введите следующее
...
**Шаг 05**: Запускаем приложение

```bash
node app.js
```

**&#9885; [Попробуйте этот пример в CodeSandbox](https://codesandbox.io/s/hello-world-in-nodejs-ue3cs3)**

<div align="right">
    <b><a href="#table-of-contents">↥ Обратно наверх</a></b>
</div>

## Q. Объясняем концепцию модуля URL в Node.js?

Модуль URL в Node.js разбивает веб-адрес на читабельные части. Используйте `require()`, чтобы
включить этот модуль. Затем парсите адрес с `url.parse()` методом, который вернет объект URL
с каждой частью адреса в качестве свойства.
